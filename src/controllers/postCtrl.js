...
module.exports = {
    ...
    async PostsByUser(req, res, next){
        try {
            const posts = await Post.find({publisher: req.body.publisher})
            .populate('publisher', ['profilePicture', 'userName', 'firstName', 'lastName']);
    ...
    async PostsByUsers(req, res, next){
        try {
            const posts = Post.find({publisher : {$in : req.body.users}})
            .populate('publisher', ['profilePicture', 'userName', 'firstName', 'lastName']);
            res.status(200).json(posts);
        }catch(err){
            next({
    ...
    async editPost(req, res, next){
        try{
            const post = await Post.updateOne({_id: req.body.id}, {$set: {content: req.body.content}});
            res.status(200).json(post);
        }catch(err){
            next({
                status:500,
